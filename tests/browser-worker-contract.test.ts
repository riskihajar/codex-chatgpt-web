...
test("native tool rows must match calls delivered to the local broker", () => {
  expect(chatGptHasUndeliveredNativeTool(2, 1)).toBe(true);
  expect(chatGptHasUndeliveredNativeTool(1, 1)).toBe(false);
  expect(chatGptHasUndeliveredNativeTool(0, 0)).toBe(false);
});

test("missing native delivery prose remains a fallback when ChatGPT strips tool identity", () => {
  expect(chatGptReportsUndeliveredNativeTool(
    "Koneksi Codex Native untuk turn ini menolak token sesi karena sudah tidak valid.",
  )).toBe(true);
  expect(chatGptReportsUndeliveredNativeTool(
    "Akses terminal Codex untuk turn ini gagal karena sesi tool sudah tidak valid.",
  )).toBe(true);
  expect(chatGptReportsUndeliveredNativeTool(
    "Akses native untuk repo sedang gagal di sesi ini.",
  )).toBe(true);
  expect(chatGptReportsUndeliveredNativeTool(
    "Akses repo lokal sedang gagal.",
  )).toBe(true);
  expect(chatGptReportsUndeliveredNativeTool(
    "Pemanggilan backend Codex gagal karena sesi turn token sudah tidak valid.",
  )).toBe(true);
  expect(chatGptReportsUndeliveredNativeTool("The GitHub token is invalid.")).toBe(false);
});
